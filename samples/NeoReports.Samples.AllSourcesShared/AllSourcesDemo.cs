...
namespace NeoReports.Samples.AllSourcesShared;

/// <summary>
/// The database seeding, column definitions and named-source registration shared by the Aspire
/// all-sources demos (samples 14 and 15). Extracted verbatim from sample 14 when sample 15 was added
/// (ADR D50/L1) so the two demos genuinely share one setup instead of duplicating ~600 lines of
/// provisioning; sample 15 is sample 14 plus the Pro packages.
/// </summary>
public static class AllSourcesDemo
{
    private const int SeedRowCount = 15_000;

    /// <summary>The Mongo database the demo seeds into.</summary>
    public const string MongoDatabaseName = "widetransactions";

    /// <summary>The Mongo collection the demo seeds into.</summary>
    public const string MongoCollectionName = "wide_transactions";

    private const string ConnectionStringPropertyKey = "connectionString";

    private const string Culture = "en-US";

    private static readonly object MongoGuidSerializerLock = new();
    private static bool _mongoGuidSerializerRegistered;

    /// <summary>
    /// Registers MongoDB.Driver's Guid representation, which the driver refuses to infer. Required
    /// before the seeding step below, which uses the driver's own typed serializer (report reads go
    /// through NeoReports' own BsonDocumentMaterializer instead, see D44).
    /// </summary>
    /// <remarks>
    /// The underlying <c>BsonSerializer.RegisterSerializer</c> is process-wide and throws on a second
    /// call for the same type, so this guards it: calling it twice, or from two hosts in one process,
    /// is a no-op rather than a <c>BsonSerializationException</c>. <see cref="EnsureMongoSeededAsync"/>
    /// calls it itself, so a host that forgets to still gets correct behaviour; hosts call it first
    /// thing anyway to keep the requirement visible at the top of <c>Program.cs</c>.
    /// </remarks>
    public static void RegisterMongoGuidSerializer()
    {
        lock (MongoGuidSerializerLock)
        {
            if (_mongoGuidSerializerRegistered)
                return;

            BsonSerializer.RegisterSerializer(new GuidSerializer(GuidRepresentation.Standard));
            _mongoGuidSerializerRegistered = true;
        }
    }

    /// <summary>Destination path template every report in both demos writes to.</summary>
    public const string LocalDestinationTemplate = "./out/{name}-{date:yyyy-MM-dd}.{ext}";

    /// <summary>The exact column list every provider-specific SELECT below shares.</summary>
    ...
    /// <summary>
    /// One working, ready-to-run typed report per database — click "Run" immediately, no setup
    /// needed. Shared by both Aspire demos so a fix to any of the four queries lands in both.
    /// </summary>
    public static void AddWideTransactionReports(IServiceCollection services, DemoConnections connections)
    {
        (string pgConnectionString, string mysqlConnectionString, string sqlServerConnectionString, string mongoConnectionString) = connections;

        services.AddReport<WideTransaction>("wide-transactions-postgres", b => b
        ...
        services.AddReport<WideTransaction>("wide-transactions-mysql", b => b
        ...
        services.AddReport<WideTransaction>("wide-transactions-sqlserver", b => b
            .From(SqlSource.Sql(
        ...
        services.AddReport<WideTransaction>("wide-transactions-mongodb", b => b
    ...
    /// <summary>The four Aspire-provisioned connection strings this demo runs against.</summary>
    public sealed record DemoConnections(string Postgres, string MySql, string SqlServer, string Mongo);

    /// <summary>
    /// Resolves all four connection strings from Aspire's <c>WithReference</c> configuration, falling
    /// back to positional command-line arguments so the Web host can also be run standalone.
    /// </summary>
    /// <param name="appHostProjectPath">Path named in the error message when nothing is configured.</param>
    public static DemoConnections ResolveConnections(string[] args, IConfiguration configuration, string appHostProjectPath)
    {
        string postgres = args.Length > 0
            ? args[0]
            : configuration.GetConnectionString("postgres-db")
                ?? throw new InvalidOperationException(
                    $"No Postgres connection string. Run via the AppHost (dotnet run --project {appHostProjectPath}) " +
                    "or pass four connection strings as arguments (postgres, mysql, sqlserver, mongo).");

        string mysqlRaw = args.Length > 1
            ? args[1]
            : configuration.GetConnectionString("mysql-db")
                ?? throw new InvalidOperationException("No MySQL connection string. See the Postgres error above for how to run this sample.");

        string sqlServer = args.Length > 2
            ? args[2]
            : configuration.GetConnectionString("sqlserver-db")
        ...
        string mongo = args.Length > 3
            ? args[3]
            : configuration.GetConnectionString("mongodb-db")
                ?? throw new InvalidOperationException("No MongoDB connection string. See the Postgres error above for how to run this sample.");

        // GuidFormat=Char36 is required on the MySQL connection string (both for seeding and for the
        // report source) so MySqlConnector reads/writes a CHAR(36) column as a native Guid instead of
        // a string - without it, RecordMaterializer's Convert.ChangeType(string, typeof(Guid)) throws,
        // since Guid doesn't implement IConvertible.
        string mysql = new MySqlConnectionStringBuilder(mysqlRaw) { GuidFormat = MySqlGuidFormat.Char36 }.ConnectionString;

        return new DemoConnections(postgres, mysql, sqlServer, mongo);
    }

    /// <summary>
    /// Everything both Aspire demos register that is not a Pro package: capability providers for
    /// every source type, the CSV/XLSX writer and Local destination factories, the Source Registry,
    /// dynamic reports, scheduling, jobs and job events.
    /// </summary>
    public static void AddSharedServices(IServiceCollection services)
    {
        services.AddNeoReportsUI();

        // A config-source provider for every source type this demo has - GET /api/capabilities is
        // never empty, so the UI's "Demo mode" fallback never shows and the Builder wizard's "Save"
        // step is never disabled. SQLite and Oracle are registered too (even though neither AppHost
        // provisions a seeded database for them - SQLite is file-based, no container needed, and
        // Oracle has no working typed report/named source here) purely so both show up as selectable
        // engine source types in the Builder wizard's step 1, same as any other registered capability.
        services.AddSqlConfigSource();
        services.AddPostgresConfigSource();
        services.AddMySqlConfigSource();
        services.AddMongoDbConfigSource();
        services.AddSqliteConfigSource();
        ...
        // .To(...)/.UploadTo(...) calls in AddWideTransactionReports - without these the Builder
        // wizard's Format/Destination steps would show "No output formats registered" even though
        // CSV/XLSX/Local work fine for the four typed reports.
        services.AddSingleton<IWriterFactory>(new CsvWriterFactory(new CsvOptions()));
        services.AddSingleton<IWriterFactory>(new XlsxWriterFactory(new XlsxOptions()));
        services.AddSingleton<IDestinationFactory>(new LocalDestinationFactory(LocalDestinationTemplate));

        // The Source Registry (D42): four named sources, seeded once the databases exist, so the
        // Builder wizard can build brand-new reports against any of them by name instead of pasting
        // a raw connection string.
        services.AddSourceRegistry();

        // Dynamic (Builder-created) reports and recurring schedules both need somewhere to persist.
        services.AddDynamicReports();
        services.AddScheduling();

        services.AddNeoReportsInMemoryJobs();
        services.AddNeoReportsArtifacts();
        services.AddPartialArtifacts(); // ADR D40 - best-effort output capture for failed/cancelled jobs
        services.AddInMemoryJobEvents(); // ADR D38 - powers the job Timeline/Retries/rate cards
    }

    /// <summary>Seeds all four databases in parallel, then registers them as named sources (D42).</summary>
    public static async Task SeedAndRegisterAsync(IServiceProvider services, DemoConnections connections)
    {
        await Task.WhenAll(
            EnsurePostgresSeededAsync(connections.Postgres),
            EnsureMySqlSeededAsync(connections.MySql),
            EnsureSqlServerSeededAsync(connections.SqlServer),
            EnsureMongoSeededAsync(connections.Mongo));

        await RegisterNamedSourcesAsync(services, connections.Postgres, connections.MySql, connections.SqlServer, connections.Mongo);
    }

    // Every column in the shared 51-column WideTransaction table/collection, reused by both Aspire
    // demos' typed report registrations.
    public static ColumnDefinition<WideTransaction>[] WideTransactionColumns() => new[]
    {
    ...
    public static async Task RegisterNamedSourcesAsync(
    ...
    public static async Task EnsurePostgresSeededAsync(string connectionString)
    {
    ...
    public static async Task EnsureMySqlSeededAsync(string connectionString)
    {
    ...
    private static async Task InsertMySqlBatchAsync(MySqlConnection connection, List<WideTransaction> rows)
    {
    ...
    public static async Task EnsureSqlServerSeededAsync(string connectionString)
    {
    ...
    private static DataTable BuildEmptySqlServerTable()
    {
    ...
    private static void AddSqlServerRow(DataTable table, WideTransaction row) => table.Rows.Add(
        row.TransactionId, row.CustomerId, row.CustomerName, row.CustomerEmail, row.CustomerCity,
        row.CustomerCountry, row.IsVipCustomer, row.ProductId, row.ProductName, row.ProductCategory,
        row.ProductSku, row.Quantity, row.UnitPrice, row.DiscountRate, row.TaxRate, row.ShippingCost,
        row.ProcessingFee, row.TotalAmount, row.Currency, row.PaymentMethod, row.IsRefunded,
    ...
    public static async Task EnsureMongoSeededAsync(string connectionString)
    {
        // Idempotent — the seeding below writes Guids through the driver's own serializer, so this
        // cannot be left to the caller to remember.
        RegisterMongoGuidSerializer();
