...
| Report detail | `/reports/{slug}` `ReportDetail` | Card, MetricCard, Chip, Banner, DataGrid, Timeline, JobStatusBadge, CatTile | `GET /api/reports/{name}` (Epic D / D4 — columns, formats, destinations, retry/failure strategy, origin). History: `GET /api/jobs?report={name}&limit=10` (Epic D / D7). Metrics/perms/changes: **mock/future** | not-found (bad slug → EmptyState), empty history (→ EmptyState) |
