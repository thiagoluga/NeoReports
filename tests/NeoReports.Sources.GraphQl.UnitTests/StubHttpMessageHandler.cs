...
/// records each request plus its body. Unlike OData's stub (which only ever needs the request URI),
/// GraphQL carries its payload in the POST body, so the body is read and captured synchronously
/// inside <see cref="SendAsync"/> — while the request is still alive — since the source's
