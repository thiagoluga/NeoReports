...
/// <summary>
/// A report defined entirely in JSON config reads from SQLite (keyset) and writes CSV, end to end.
/// The dynamic <c>"sqlite"</c> source materializes positional <see cref="ReportRecord"/> rows by
/// schema-column name, reusing the shared ADO.NET keyset engine and the existing pipeline.
/// </summary>
public class DynamicConfigSqliteTests : IClassFixture<SqliteFileFixture>, IDisposable
{
    private readonly SqliteFileFixture _fixture;
    private readonly string _outDir = Path.Combine(Path.GetTempPath(), "nr-dyn-sqlite", Guid.NewGuid().ToString("N"));

    public DynamicConfigSqliteTests(SqliteFileFixture fixture) => _fixture = fixture;

    [Fact]
    public async Task Config_drives_sqlite_source_to_csv_end_to_end()
    {
        var json = $$"""
        {
          "name": "sales-dyn",
    ...
    public void Provider_requires_connection_string_sql_and_key()
    {
        ...
        Should.Throw<ConfigurationException>(() => provider.Create(new SourceConfig("sqlite"), schema, services));

        var partial = new Dictionary<string, object?> { ["connectionString"] = "Data Source=x", ["sql"] = "SELECT 1" };
        Should.Throw<ConfigurationException>(() => provider.Create(new SourceConfig("sqlite", partial), schema, services));
    }

    [Fact]
    public void AddSqliteConfigSource_registers_the_provider_and_health_check()
    {
        var services = new ServiceCollection();
        services.AddSqliteConfigSource();
        using var provider = services.BuildServiceProvider();

        provider.GetServices<IConfigSourceProvider>().ShouldContain(p => p.Type == "sqlite");
    }
