...
/// <summary>
/// ADR D42/D43: <c>Source.MySqlNamed</c> resolves its connection through the source registry
...
public class NamedMySqlSourceTests : IClassFixture<MySqlServerFixture>
{
    ...
    public NamedMySqlSourceTests(MySqlServerFixture fixture) => _fixture = fixture;
    ...
    public async Task Switching_the_registered_connection_string_redirects_the_very_next_run()
    {
        ...
        await registry.SaveAsync(new SourceDefinition("sales-db", "mysql",
        ...
        var source = Source.MySqlNamed("sales-db", Sql).Keyset<Sale, long>(v => v.Id, pageSize: 10);
    ...
    {
        // The default app user has no cross-database privileges (only its own MYSQL_DATABASE), so
        // creating and seeding a second database goes through the root connection.
        await using (var connection = new MySqlConnection(_fixture.RootConnectionString))
        {
            await connection.OpenAsync();
            await using var create = connection.CreateCommand();
            create.CommandText = "CREATE DATABASE IF NOT EXISTS salesalt;";
            await create.ExecuteNonQueryAsync();
        }

        var builder = new MySqlConnectionStringBuilder(_fixture.RootConnectionString) { Database = "salesalt" };
        var altConnectionString = builder.ConnectionString;

        await using var altConnection = new MySqlConnection(altConnectionString);
        await altConnection.OpenAsync();
        await using var seed = altConnection.CreateCommand();
        seed.CommandText = """
            CREATE TABLE IF NOT EXISTS Sales (Id BIGINT PRIMARY KEY, Customer VARCHAR(100) NOT NULL, Amount DECIMAL(18,2) NOT NULL, Date DATETIME NOT NULL);
            INSERT INTO Sales (Id, Customer, Amount, Date)
            SELECT 9001, 'AltCo', 1.00, '2026-02-02'
