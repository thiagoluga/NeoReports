...
/// <summary>
/// A report defined entirely in JSON config reads from MongoDB (keyset) and writes CSV, end to end.
...
public class DynamicConfigMongoDbTests : IDisposable
{
    ...
    public DynamicConfigMongoDbTests(MongoDbServerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task Config_drives_mongodb_source_to_csv_end_to_end()
    {
    ...
    [Fact]
    public void Provider_requires_connection_string_database_collection_and_key()
    {
        var provider = new MongoDbConfigSourceProvider();
        var schema = new ReportSchema(new[] { new ReportColumn("Id", ColumnType.Integer) });
        using var services = new ServiceCollection().BuildServiceProvider();

        Should.Throw<ConfigurationException>(() => provider.Create(new SourceConfig("mongodb"), schema, services));

        var partial = new Dictionary<string, object?> { ["connectionString"] = "mongodb://x", ["database"] = "salesdb" };
        Should.Throw<ConfigurationException>(() => provider.Create(new SourceConfig("mongodb", partial), schema, services));
    }

    [Fact]
    public void AddMongoDbConfigSource_registers_the_provider_and_health_check()
    {
        var services = new ServiceCollection();
        services.AddMongoDbConfigSource();
        using var provider = services.BuildServiceProvider();

        provider.GetServices<IConfigSourceProvider>().ShouldContain(p => p.Type == "mongodb");
    }
