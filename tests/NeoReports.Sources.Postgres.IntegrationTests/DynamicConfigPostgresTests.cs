...
/// <summary>
/// A report defined entirely in JSON config reads from PostgreSQL (keyset) and writes CSV, end to
...
public class DynamicConfigPostgresTests : IDisposable
{
    ...
    public DynamicConfigPostgresTests(PostgresServerFixture fixture) => _fixture = fixture;

    [SkippableFact]
    public async Task Config_drives_postgres_source_to_csv_end_to_end()
    {
        ...
        var json = $$"""
        {
          ...
          "source": {
            ...
            "properties": {
              ...
              "sql": "SELECT Id, Customer, Amount, Date FROM Sales WHERE (@cursor IS NULL OR Id > @cursor::bigint) ORDER BY Id",
              "key": "Id",
              "pageSize": 1000
            }
          },
    ...
    public void Provider_requires_connection_string_sql_and_key()
    {
        ...
        Should.Throw<ConfigurationException>(() => provider.Create(new SourceConfig("postgres"), schema, services));

        var partial = new Dictionary<string, object?> { ["connectionString"] = "Host=.", ["sql"] = "SELECT 1" };
        Should.Throw<ConfigurationException>(() => provider.Create(new SourceConfig("postgres", partial), schema, services));
    }

    [Fact]
    public void AddPostgresConfigSource_registers_the_provider_and_health_check()
    {
        var services = new ServiceCollection();
        services.AddPostgresConfigSource();
        using var provider = services.BuildServiceProvider();

        provider.GetServices<IConfigSourceProvider>().ShouldContain(p => p.Type == "postgres");
    }
