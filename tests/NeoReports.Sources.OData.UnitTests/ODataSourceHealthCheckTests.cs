...
/// <summary>Tests the OData source's on-demand health check (ADR D42/D62, <c>type: "odata"</c>).</summary>
public sealed class ODataSourceHealthCheckTests
{
    ...
    private static SourceDefinition Definition(string url, string? healthCheckPath = null)
    {
        ...
        return new SourceDefinition("odata-source", "odata", properties);
    }
    ...
    public async Task Healthy_when_the_HEAD_request_succeeds()
    {
        ...
        var check = new ODataSourceHealthCheck();
    ...
    [Fact]
    public async Task Probes_the_configured_health_check_path_relative_to_the_resource_url()
    {
        Uri? seenUri = null;
        HttpClient client = StubHttpMessageHandler.CreateClient(request =>
        {
    ...
    {
        var check = new ODataSourceHealthCheck();
        var definition = new SourceDefinition("odata-source", "odata", new Dictionary<string, object?>());
